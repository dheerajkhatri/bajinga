...
<head>
<meta charset="utf-8">
<title>SQL View - Creating, Updating Views | DBMS Tutorial | Studytonight</title>
<meta name="viewport" content="width=device-width, initial-scale=1">
<meta name="keywords" content="Database technology Notes, RDBMS, SQL Query, Select Query, Insert Query, Delete Query, Update Query, Codd Rule, E-R diagram, Database Model, Normalization,View,Outer Join,Cross Join, SQL Sequence, SQL Constraints, Database View, table, row, SQL Join"/> 
<meta name="description" content="A view in SQL is logical subset of data from one or more table. View is used to restricts data access."/>
...
<a href="sql-views.php#" id="slide-toggle-sidebar"><span class="pull-right"><img src="../css/resource/collapsed-menu.png" alt="Menu"/></span><span>SEE THE INDEX</span></a>
</div>
...
<pre>
CREATE or REPLACE <b>view</b> <i>view_name</i> AS
SELECT <i>column_name</i>(s)
FROM <i>table_name</i>
WHERE <i>condition</i>
</pre>
<hr/>
<h4>Example of Creating a View</h4>
...
<p>SQL Query to Create View</p>
<pre>
CREATE or REPLACE <b>view</b> sale_view as select * from Sale where customer = 'Alex';
</pre>
<p>The data fetched from select statement will be stored in another object called <b>sale_view</b>. We can use create seperately and replace too but using both together works better.</p>
<hr/>
<h4>Example of Displaying a View</h4>
<p>Syntax of displaying a view is similar to fetching data from table using Select statement.</p>
<pre>
SELECT * from <b>sale_view</b>;
</pre>
<hr/>
<h4>Force View Creation</h4>
<p>force keyword is used while creating a view. This keyword force to create View even if the table does not exist. After creating a force View if we create the base table and enter values in it, the view will be automatically updated.</p>
<p>Syntax for forced View is,</p>
<pre>
CREATE or REPLACE <i>force</i> <b>view</b> <i>view_name</i> AS
SELECT <i>column_name</i>(s)
FROM <i>table_name</i>
WHERE <i>condition</i>
</pre>
<hr/>
<h4>Update a View</h4>
<p>Update command for view is same as for tables.</p>
<p>Syntax to Update a View is,</p>
<pre>
UPDATE <b>view-name</b> 
set value
WHERE condition;
</pre>
<p>If we update a view it also updates base table data automatically.</p>
<hr/>
...
SELECT <i>column_name</i>(s)
FROM <i>table_name</i>
WHERE <i>condition</i> with <b>read-only</b>
</pre>
<p>The above syntax will create view for read-only purpose, we cannot Update or Insert data into read-only view. It will throw an error.</p>
<hr/>
<h4>Types of View</h4>
