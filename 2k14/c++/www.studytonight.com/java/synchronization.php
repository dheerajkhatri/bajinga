...
<title>Synchronization of threads in Java Multithreading | Core Java Tutorial | Studytonight</title>
...
<meta name="description" content="At times when more than one thread try to access a shared resource, we need to ensure that resource will be used by only one thread at a time. The process by which this is achieved is called synchronization."/>
...
<a href="synchronization.php#" id="slide-toggle-sidebar"><span class="pull-right"><img src="../css/resource/collapsed-menu.png" alt="Menu"/></span><span>SEE THE INDEX</span></a>
</div>
...
<hr/>
<h4>Why we use Syncronization ?</h4>
...
<p>Consider an example, Suppose we have two different threads <b>T1</b> and <b>T2</b>, T1 starts execution and save certain values in a file <i>temporary.txt</i> which will be used to calculate some result when T1 returns. Meanwhile, T2 starts and before T1 returns, T2 change the values saved by T1 in the file temporary.txt (temporary.txt is the shared resource). Now obviously T1 will return wrong result.</p>
<p>To prevent such problems, synchronization was introduced. With synchronization in above case, once T1 starts using <i>temporary.txt</i> file, this file will be <b>locked</b>(LOCK mode), and no other thread will be able to access or modify it until T1 returns.</p>
<hr/>
<h4>Using Synchronized Methods</h4>
<p>Using Synchronized methods is a way to accomplish synchronization. But lets first see what happens when we do not use synchronization in our program.</p><br/>
<h4>Example with no Synchronization</h4>
<pre>
class First
{
...
public class Syncro
{
 ...
 {
  <i>First <b>fnew</b> = new First();</i>
  Second ss = new second(<b>fnew</b>, "welcome");
  Second ss1= new second (<b>fnew</b>,"new");
  Second ss2 = new second(<b>fnew</b>, "programmer");
 }
}
</pre>
<pre>
...
<p>In the above program, object <b>fnew</b> of class First is shared by all the three running threads(ss, ss1 and ss2) to call the shared method(<i>void</i> <b>display</b>). Hence the result is unsynchronized and such situation is called <b>Race condition</b>.</p><br/>
<h4>Synchronized Keyword</h4>
<p>To synchronize above program, we must <i>serialize</i> access to the shared <b>display()</b> method, making it available to only one thread at a time. This is done by using keyword <b>synchronized</b> with display() method.</p>
<pre>
<b>synchronized</b> <i>void</i> <b>display</b> (String msg) 
</pre>
<hr/>
<h4>Using Synchronised block</h4>
<p>If you have to synchronize access to object of a class that has no synchronized methods, and you cannot modify the code. You can use synchronized block to use it.</p>
<pre>
class First
{
 public void display(String msg)
...
class Second extends Thread
{
 ...
 public void run()
 {
  <b>synchronized(fobj)</b>       <font color="green">//Synchronized block</font>
  {
   <i>fobj.display(msg)</i>;
  }
 }
}
...
<div><ul class="pager">
<li class="previous">
<a href="joining-a-thread.php">&larr; Prev</a>
</li>
<li class="next">
<a href="interthread-communication.php">Next &rarr;</a>
</li>
</ul></div>
</div>
</div>
