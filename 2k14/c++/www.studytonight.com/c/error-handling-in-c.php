...
<title>Error Handling in C | C Language Tutorial | Studytonight</title>
<meta name="description" content="C language does not provide direct support for error handling. However few method defined in error.h header file can be used to point out error using return value of the function call">
...
<a href="error-handling-in-c.php#" id="slide-toggle-sidebar"><span class="pull-right"><img src="../css/resource/collapsed-menu.png" alt="Menu"/></span><span>SEE THE INDEX</span></a>
</div>
...
C language does not provide direct support for error handling. However few method and variable defined in <b>error.h</b> header file can be used to point out error using return value of the function call. In C language, a function return -1 or NULL value in case of any error and a global variable <b>errno</b> is set with the error code. So the return value can be used to check error while programming.  
</p>
<p>C language uses the following functions to represent error</p>
<ul class="content">
<li><b>perror()</b> return string pass to it along with the textual represention of current errno value.</li>
<li><b>strerror()</b> is defined in <b>string.h</b> library. This method returns a pointer to the string representation of the current errno value.</li>
</ul>
<hr/>
<h4>Example</h4>
<pre>
...
main( )
{
  char *ptr = malloc( 1000000000UL);  <i>//requesting to allocate 1gb memory space</i>  
  if ( ptr == NULL )        <span style="color:red">//if memory not available, it will return null</span>  
  {  
     puts("malloc failed");
     puts(strerror(errno));
     exit(EXIT_FAILURE);	<span style="color:red">//exit status failure</span>       
  }
  else
  {
     free( ptr);
     exit(EXIT_SUCCESS);	<i>//exit status Success</i>       
  }
}
</pre>
<p>
Here exit function is used to indicate exit status. Its always a good practice to exit a program with a exit status. <b>EXIT_SUCCESS</b> and <b>EXIT_FAILURE</b> are two macro used to show exit status. In case of program coming out after a successful operation <b>EXIT_SUCCESS</b> is used to show successfull exit. It is defined as 0. <b>EXIT_Failure</b> is used in case of any failure in the program. It is defined as -1.</p>
<hr/>
<div><ul class="pager">
<li class="previous">
<a href="file-input-output.php">&larr; Prev</a>
</li>
<li class="next">
<a href="dynamic-memory-allocation-in-c.php">Next &rarr;</a>
</li>
</ul></div>
</div>
</div>
