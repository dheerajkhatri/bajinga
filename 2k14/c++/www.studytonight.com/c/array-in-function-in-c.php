...
<title>Passing Array to Function | C Language Tutorial | Studytonight</title>
...
<meta name="description" content="Whenever we need to pass a list of elements as argument to the function, it is prefered to do so using an array. But how can we pass an array as argument ? Lets see how to do it"/>
...
<a href="array-in-function-in-c.php#" id="slide-toggle-sidebar"><span class="pull-right"><img src="../css/resource/collapsed-menu.png" alt="Menu"/></span><span>SEE THE INDEX</span></a>
</div>
...
<div class="span9">
<div id="body-content">
<h3>Passing Array to Function</h3> 
<p>Whenever we need to pass a list of elements as argument to the function, it is prefered to do so using an array. But how can we pass an array as argument ? Lets see how to do it.</p>
<hr/>
<h4>Declaring Function with array in parameter list</h4>
There are two possible ways to do so, one will lead to call by value and the other is used to perform call be reference.
<ul class="content">
<li>We can either have an array in parameter.
<pre>
int sum (int arr[]);
</pre></li>
<li>Or, we can have a pointer in the parameter list, to hold base address of array.
<pre>
int sum (int* ptr);
</pre>
<p>We will study this in detail later when we will study pointers.</p>
</li>
</ul>
<hr/>
<h4>Returning Array from function</h4>
<p>We dont&apos;t return an array from functions, rather we return a pointer holding the base address of the array to be returned. But we must, make sure that the array exists after the function ends.</p>
<pre>int* sum (int x[])
{
 //statements
 return x ;
}</pre>
<p>We will discuss about this when we will study pointers with arrays.</p>
<hr/>
<h4>Example :  Create a function to sort an Array of elements</h4>
<pre>
...
<ul class="content">
<li>return type is <b>void</b>, that means function does not return any thing.</li>
<li><b>Sorting</b> is the function name.</li>
<li>int x[ ] and int y is the <b>parameter list</b>.</li>
<li>int i, j, temp inside curly braces are the local variable declaraction.</li>
</ul> 
</p>
...
<div><ul class="pager">
<li class="previous">
<a href="types-of-function-calls.php">&larr; Prev</a>
</li>
<li class="next">
<a href="structures-in-c.php">Next &rarr;</a>
</li>
</ul></div>
</div>
</div>
