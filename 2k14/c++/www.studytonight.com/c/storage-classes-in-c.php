...
<title>Storage Classes in C Language | C Language Tutorial | Studytonight</title>
<meta name="description" content="In C language, each variable has a <b>storage class</b> which decides scope, visibility and lifetime of that variable. The following storage classes are most oftenly used in C programming">
...
<a href="storage-classes-in-c.php#" id="slide-toggle-sidebar"><span class="pull-right"><img src="../css/resource/collapsed-menu.png" alt="Menu"/></span><span>SEE THE INDEX</span></a>
</div>
...
<p>
In C language, each variable has a <b>storage class</b> which decides scope, visibility and lifetime of that variable. The following storage classes are most oftenly used in C programming,</p>
<ol class="content">
<li><b>Automatic variables</b></li>
<li><b>External variables</b></li>
<li><b>Static variables</b></li>
<li><b>Register variables</b></li>
</ol>
<hr/>
<h4>Automatic variables</h4>
<p>A variable declared inside a function without any storage class specification, is by default an <b>automatic variable</b>. They are created when a function is called and are destroyed <b>automatically</b> when the function exits. Automatic variables can also be called local variables because they are local to a function. By default they are assigned <b>garbage value</b> by the compiler.</p> 
<pre>
void main()
{
 int detail;
 or 
 auto int detail;    <i>//Both are same</i>
}
</pre>
</p>
<hr/>
<h4>External or Global variable</h4>
<p>
A variable that is declared outside any function is a <b>Global variable</b>. <b>Global</b> variables remain available throughout the entire program. One important thing to remember about global variable is that their values can be changed by any function in the program.</p>
<pre>
int number;
void main()
{
...
Here the global variable <b>number</b> is available to all three functions.
</p>
<hr/>
<h4>extern keyword</h4>
<p>The <b>extern</b> keyword is used before a variable to inform the compiler that this variable is declared somewhere else. The <b>extern</b> declaration does not allocate storage for variables.
</p>
<p class="center"><img src="images/extern-keyword-in-c.gif" alt="extern keyword in c"/></p>

<h4>Problem when extern is not used</h4>
<pre>
main()
{
  a = 10;      <span style="color:red">//Error:cannot find variable a</span>
  printf("%d",a);    
}
</pre>
...
main()
{
  <b>extern</b> int x;  <i>//Tells compiler that it is defined somewhere else</i>
  x = 10;      
  printf("%d",x);    
}

int x;   <i> //Global variable x</i>
</pre>

<hr/>
<h4>Static variables</h4>
<p>
A <b>static</b> variable tells the compiler to persist the variable until the end of program. Instead of creating and destroying a variable every time when it comes into and goes out of scope, <b>static</b> is initialized only once and remains into existence till the end of program. A static variable can either be internal or external depending upon the place of declaraction. Scope of <b>internal static</b> variable remains inside the function in which it is defined. <b>External static</b> variables remain restricted to scope of file in each they are declared.</p>
<p>They are assigned <b>0 (zero)</b> as default value by the compiler.</p>
<p>
<pre>
void test();   <font color="green">//Function declaration (discussed in next topic)</font>
...
void test()
{
 static int a = 0;        <font color="green">//Static variable</font>
 a = a+1;
 printf("%d\t",a);
}
<b>output :</b>
...
<hr/>
<h4>Register variable</h4>
<p>
<b>Register</b> variable inform the compiler to store the variable in register instead of memory. <b>Register</b> variable has faster access than normal variable. Frequently used variables are kept in register. Only few variables can be placed inside register.</p>
<p><b>NOTE : </b>We can never get the address of such variables.</p>
<p><b>Syntax :</b></p>
<pre>register int number;</pre></p>
<hr/>
<div><ul class="pager">
<li class="previous">
<a href="string-and-character-array.php">&larr; Prev</a>
</li>
<li class="next">
<a href="user-defined-functions-in-c.php">Next &rarr;</a>
</li>
</ul></div>
</div>
</div>
