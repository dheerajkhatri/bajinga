...
<title>How a Servlet Application works | Servlet Tutorial | Studytonight</title>

<meta name="keywords" content="Servlet tutotials, Servlet technology notes, Advance Java tutorials, J2EE notes, Servlet request, Servlet response, Web application, Servlet Context, HttpSession, Attributes, Servlet Config, Servlet Life cycle, Filters, Session, Servlet Events and Listeners, Cookies in Servlet, Servlet Application Working, Servlet Application Life Cycle" />
<meta name="description" content="How a Servlet Application works? Web container is responsible for managing execution of servlet and Jsp pages for Java EE application. When a request comes in for a servlet, the server hands the request to Web Container."/>
...
<a href="how-a-servlet-application-work.php#" id="slide-toggle-sidebar"><span class="pull-right"><img src="../css/resource/collapsed-menu.png" alt="Menu"/></span><span>SEE THE INDEX</span></a>
</div>
...
<div id="body-content">
<h3>How a Servlet Application works</h3>
<p><b>Web container</b> is responsible for managing execution of servlets and JSP pages for Java EE application.</p>
<p>When a request comes in for a servlet, the server hands the request to the Web Container. <b>Web Container</b> is responsible for instantiating the servlet or creating a new thread to handle the request. Its the job of Web Container to get the request and response to the servlet. The container creates multiple threads to process multiple requests to a single servlet. </p>
<p>
<b>Servlets don't have a main() method</b>. Web Container manages the life cycle of a Servlet instance.
</p>
<hr/>
<h4>Quick Revision on How a Servlet works</h4>
<ol class="content">
<li>User sends request for a servlet by clicking a link that has URL to a servlet.</li>
<p class="center"><img src="images/how-a-servlet-works-step1.jpg" alt="how a servlet application works" /></p>

<li>The container finds the servlet using <b>deployment descriptor</b> and creates two objects :</li>
<ol type='a'>
<li><b>HttpServletRequest</b></li>
<li><b>HttpServletResponse</b></li>
</ol>
<p class="center"><img src="images/how-a-servlet-works-step2.jpg" alt="Request and Response objects created while servlet execution" /></p>

<li>Then the container creates or allocates a thread for that request and calls the Servlet's <code>service()</code> method and passes the <b>request, response</b> objects as arguments.</li>
<p class="center"><img src="images/how-a-servlet-works-step3.jpg" alt="call to service() method with request and response object" /></p>

<li>The <code>service()</code> method, then decides which servlet method, <code>doGet()</code> or <code>doPost()</code> to call, based on <b>HTTP Request Method</b>(Get, Post etc) sent by the client. Suppose the client sent an HTTP GET request, so the <code>service()</code> will call Servlet's <code>doGet()</code> method.</li>
<p class="center"><img src="images/how-a-servlet-works-step4.jpg" alt="call to doGet() or doPost() in Servlet execution" /></p>

<li>Then the Servlet uses response object to write the response back to the client.</li>
<p class="center"><img src="images/how-a-servlet-works-step5.jpg" alt="Sending back the response to the client after servlet execution" /></p>

<li>After the <code>service()</code> method is completed the <b>thread</b> dies. And the request and response objects are ready for <b>garbage collection</b>.</li>
<p class="center"><img src="images/how-a-servlet-works-step6.jpg" alt="End of Servlet Execution" /></p>
</ol>
<hr/>
<div><ul class="pager">
<li class="previous">
<a href="servlet-api">&larr; Prev</a>
</li>
  <li class="next">
    <a href="servlet-life-cycle">Next &rarr;</a>
  </li>
</ul></div>
