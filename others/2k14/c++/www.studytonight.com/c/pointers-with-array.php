...
<title>Pointers with Arrays in C | C Language Tutorial | Studytonight</title>
<meta name="description" content="When an array is declared, compiler allocates sufficient amount of memory to contain all the elements of the array. Base address which gives location of the first element is also allocated by the compiler">
...
<a href="pointers-with-array.php#" id="slide-toggle-sidebar"><span class="pull-right"><img src="../css/resource/collapsed-menu.png" alt="Menu"/></span><span>SEE THE INDEX</span></a>
</div>
...
<p>
Suppose we declare an array <b>arr</b>,</p>
<pre>int arr[5]={ 1, 2, 3, 4, 5 };</pre> 
<p>Assuming that the base address of <b>arr</b> is 1000 and each integer requires two byte, the five element will be stored as follows
</p>
<p class="center"><img src="images/array-in-pointer.jpg" alt="address of array"/></p>
<p>
Here variable <b>arr</b> will give the base address, which is a constant pointer pointing to the element, <b>arr[0]</b>. Therefore <b>arr</b> is containing the address of <b>arr[0]</b> i.e 1000.
<pre>
 <b>arr</b> <i>is equal to</i> <b>&arr[0]</b>   // by default
</pre>
</p>
<p>
We can declare a pointer of type <code>int</code> to point to the array <b>arr</b>.</p>
<pre>
int *p;
p = arr;  
or p = &arr[0];  <font color="green">//both the statements are equivalent.</font>
</pre>   
<p>Now we can access every element of array <b>arr</b> using <b>p++</b> to move from one element to another.
</p>
<p><b>NOTE :</b> You cannot decrement a pointer once incremented. <code>p--</code> won&apos;t work.</p>
<hr/>
<h4>Pointer to Array</h4>
<p>As studied above, we can use a pointer to point to an Array, and then we can use that pointer to access the array. Lets have an example,</p>
<pre>
int i;
int a[5] = {1, 2, 3, 4, 5};
int *p = a;  <i>// same as int*p = &a[0]</i>
for (i=0; i<5; i++)
{
 printf("%d", *p);
 p++;
...
<p>In the aboce program, the pointer <b>*p</b> will print all the values stored in the array one by one. We can also use the Base address (<b>a</b> in above case) to act as pointer and print all the values.</p>
<p class="center"><img src="images/pointer-to-array-details.gif" alt="Array using pointer tips"/></p>
<hr/>
<h4>Pointer to Multidimensional Array</h4>
<p>A multidimensional array is of form, <code>a[i][j]</code>. Lets see how we can make a pointer point to such an array. As we know now, name of the array gives its base address. In <code>a[i][j]</code>, <b>a</b> will give the base address of this array, even <code>a+0+0</code> will also give the base address, that is the address of <b>a[0][0]</b> element.</p>
<p>Here is the generalized form for using pointer with multidimensional arrays.</p>
<pre>
<b>*(*(ptr + i) + j)</b> <i>is same as</i> <b>a[i][j]</b></pre>
<hr/>
<h4>Pointer and Character strings</h4>
<p>
Pointer can also be used to create strings. Pointer variables of <b>char</b> type are treated as string.</p>
<pre>char *str = "Hello";</pre>
<p>This creates a string and stores its address in the pointer variable <b>str</b>. The pointer <b>str</b> now points to the first character of the string "Hello". Another important thing to note that string created using <b>char</b> pointer can be assigned a value at <b>runtime</b>.
<pre>
char *str;
str = "hello";   <i>//thi is Legal</i>
</pre>
<p>The content of the string can be printed using <code>printf()</code> and <code>puts()</code>.</p>
<pre>
printf("%s", str);
puts(str);
</pre> 
<p>Notice that <b>str</b> is pointer to the string, it is also name of the string. Therefore we do not need to use indirection operator <code>*</code>.
</p>
<hr/>
...
<i>//Now see same array without using pointer</i>
char name[3][20]= { 
		    "Adam",
		    "chris",
		    "Deniel"
...
<p class="center"><img src="images/benefit-of-pointer-with-char-array.gif" alt="Pointer with character array in c"/></p>
In the second approach memory wastage is more, hence it is prefered to use pointer in such cases. 
</p>
<hr/>
<div><ul class="pager">
<li class="previous">
<a href="declaring-and-initializing-pointer.php">&larr; Prev</a>
</li>
<li class="next">
<a href="pointers-to-structure-in-c.php">Next &rarr;</a>
</li>
</ul></div>
</div>
</div>
